...
const PROPOSITO_DURACAO_MAXIMA = "Para configurar a duração máxima do jogo";
/* ------------------------------------------------------------------------- */
...
const RESULTADO_CANCELOU = "Cancelou o Jogo Atual"
/* ------------------------------------------------------------------------- */
...
function mostraConfiguracaoJogo() {
  ...
  document.getElementById(TD_DURACAO_MAXIMA).innerHTML = configuracao.duracaoMaxima;
}
...
function pedeDuracaoMaxima() {
  configuracao.duracaoMaxima = pedeNumeroInteiro(configuracao.duracaoMinima, Infinity, PROPOSITO_DURACAO_MAXIMA);
...
function fazTentativa() {
  ...
  if (valorTentativa == jogo.numeroAleatorio) {
    ...
    resultadoTentativa = RESULTADO_CANCELOU;
  }
 ...
 */
 ...
 //var sec = 0;
 //function mostraTempoJogo ( val ) { return val > 9 ? val : "0" + val; }
 //setInterval( function(){
  //   document.getElementById(SPAN_TEMPO_JOGO).innerHTML=mostraTempoJogo(++sec%60);
 //}, 1000);
...
function iniciaJogo() {
  ...
  var sec = 0
  //temporizadorTempoJogo = setInterval(function mostraTempoJogo() { 
    //document.getElementById(SPAN_TEMPO_JOGO).innerHTML=(++sec%60);
  //}, 1000);

  temporizadorTempoRestante = setInterval(function mostraTempoJogo() { 
    document.getElementById(SPAN_TEMPO_RESTANTE).innerHTML = (configuracao.duracaoMaxima - (++sec%60));
  }, 1000);

  temporizadorDuracaoMaxima = setTimeout(function() {terminaJogo("TempOver")}, (configuracao.duracaoMaxima * 1000) + 1);
  
  
  //temporizadorTempoJogo - jogo.inicio;
...
function terminaJogo(resultado) {
  ...
  //clearTimeout(temporizadorTempoJogo);
  clearTimeout(temporizadorTempoRestante);
  ...
  } else if (resultado == RESULTADO_CANCELOU) {
    alert(RESULTADO_CANCELOU + " após " + jogo.tentativas.length + " tentativas.")
  } else if (resultado == "TempOver") {
    alert("O Tempo chegou ao fim...")
  } else {
